...
def increase_fs(module, output, dest):
    ...
    msg = 'Cannot increase filesystem: {}.'.format(mount_point)
...
def unzip(module, output, src, dst, resize_fs=True):
    ...
    try:
        ...
        if resize_fs and increase_fs(module, output, dst):
            ...
            msg = 'Cannot unzip {}, exception:{}'.format(src, exc)
            module.log(msg)
            output['messages'].append(msg)
            return False
    return True
...
def remove_efix(module, output, machine):
    ...
    """
    failed_rm = 0
    module.debug('{}: Removing all installed efix'.format(machine))
    ...
    if rc != 0:
        msg = 'Cannot remove efix on {}, command \'{}\' returned rc: {}'.format(machine, ' '.join(cmd), rc)
        output['messages'].append(msg)
    ...
    for line in stdout.splitlines():
        ...
        if match:
            if 'SUCCESS' in match.group(2):
                ...
                failed_rm += 1
    if failed_rm or rc:
        msg = 'failed to remove {} efix(es)'.format(failed_rm)
        output['messages'].append(msg)
        module.log('[WARNING] {}: {}'.format(machine, msg))
        module.log('[WARNING] {}: stderr: {}, stdout: {}'.format(machine, stdout, stderr))
...
def check_epkgs(module, output, machine, epkg_list, lpps, efixes):
    ...
    module.debug('{}: locked_files: {}'.format(machine, locked_files))
    ...
    for epkg_path in epkg_list:
        ...
        for line in stdout.splitlines():
            ...
            if prereq not in lpps:
                ...
                module.log('{}: reject {}'.format(machine, epkg['reject']))
                break  # stop parsing
        ...
        if epkg['pkg_date']:
            ...
            if sec_from_epoch == -1:
                module.log('[WARNING] {}: {}: "{}" for epkg:{} '.format(machine, msg, epkg['pkg_date'], epkg))
    ...
    for epkg in sorted_epkgs:
        if set(epkgs_info[epkg]['files']).isdisjoint(set(global_file_locks)):
            ...
            output['messages'].append('a previous efix to install will lock a file of {} '
            ...
            epkgs_info[epkg]['reject'] = '{}: locked by previous efix to install'\
                                         .format(os.path.basename(epkgs_info[epkg]['path']))
            module.log('{}: reject {}'.format(machine, epkgs_info[epkg]['reject']))
...
def parse_lpps_info(module, output, machine):
    ...
    with open(os.path.abspath(os.path.join(os.sep, lslpp_file)), 'r') as myfile:
        for myline in myfile:
            ...
            mylist = myline.split(':')
            if len(mylist) < 3:
                msg = 'file {} is malformed: got line: "{}"'.format(lslpp_file, myline)
                module.log('{}: {}'.format(machine, msg))
                output['messages'].append(msg)
                continue
...
def run_flrtvc(module, output, machine, flrtvc_path, params, force):
    ...
    if not os.path.exists(lslpp_file) or not os.path.exists(emgr_file):
        if not os.path.exists(lslpp_file):
            output['messages'].append('Failed to list filsets (lslpp), {} does not exist'
                                      .format(lslpp_file))
        if not os.path.exists(emgr_file):
            output['messages'].append('Failed to list fixes (emgr), {} does not exist'
                                      .format(emgr_file))
        return False
    ...
    module.debug('{}: run flrtvc in compact mode: cmd="{}"'.format(machine, ' '.join(cmd)))
    rc, stdout, stderr = module.run_command(cmd)
    if rc != 0 and rc != 2:
        ...
        module.log('stdout:{}'.format(stdout))
        module.log('stderr:{}'.format(stderr))
        output['messages'].append(msg + " stderr: {}".format(stderr))
        return False
    ...
    if params['save_report']:
        ...
        with open(filename, 'w') as myfile:
            ...
            module.debug('{}: write flrtvc report to file, cmd "{}"'.format(machine, ' '.join(cmd)))
            rc, stdout, stderr = module.run_command(cmd)
...
def run_parser(module, machine, output, report):
    ...
    module.debug('{}: extract {} urls in the report'.format(machine, len(rows)))
...
def run_downloader(module, machine, output, urls, resize_fs=True):
    ...
    for url in urls:
        ...
        module.debug('{}: protocol={}, srv={}, rep={}, name={}'.format(machine, protocol, srv, rep, name))

        if '.epkg.Z' in name:  # URL as an efix file
            module.debug('{}: treat url as an epkg file'.format(machine))
            ...
            module.debug('{}: treat url as a tar file'.format(machine))
            ...
            if download(module, out, url, dst, resize_fs):
                ...
                module.debug('{}: found {} epkg.Z file in tar file'.format(machine, len(epkgs)))
                    ...
                    for attempt in range(3):
                        try:
                            ...
                            if resize_fs:
                                ...
                                msg = 'Cannot extract tar file {} to {}: {}'.format(epkg, tar_dir, exc)
                                module.log('[WARNING] {}: {}', machine, msg)
            ...
            module.debug('{}: treat url as a directory'.format(machine))
            ...
            out['2.discover'].extend(epkgs)
            module.debug('{}: found {} epkg.Z file in html body'.format(machine, len(epkgs)))
...
def run_installer(module, machine, output, epkgs, resize_fs=True):
        ...
        for attempt in range(3):
            try:
                ...
                if resize_fs:
                    ...
                    msg = 'Cannot copy file {} to {}: {}'.format(epkg, destpath, exc)
                    module.log('[WARNING] {}: {}', machine, msg)
                    output['messages'].append(msg)
                    break
            else:
    ...
    if rc > 0:
        ...
        cmd += [' -a', 'location={}'.format(destpath)]
        cmd += ['-a', 'packages=all', lpp_source]
        rc, stdout, stderr = module.run_command(cmd)
        if rc != 0:
            msg = 'Cannot define NIM lpp_source resource {} for location \'{}\''.format(lpp_source, destpath)
            module.log('[WARNING] {}: {}', machine, msg)
            output['messages'].append(msg)
            return False
    ...
    if rc == 0:
        ...
        if 'master' in nimtype:
            ...
            cmd = '/usr/sbin/nim -o cust -a lpp_source={} -a filesets="{}" {}'.format(lpp_source, efixes, machine)
        elif 'vios' in nimtype:
            cmd = '/usr/sbin/nim -o updateios -a preview=no -a lpp_source={} {}'.format(lpp_source, machine)

        rc, stdout, stderr = module.run_command(cmd)
        module.debug('{}: customization result is {}'.format(machine, stdout))
        if rc == 0:
            install_ok = True
        ...
        msg = 'Cannot list NIM resource for \'{}\''.format(machine)
    ...
    if rc == 0:
        ...
        rc, stdout, stderr = module.run_command(cmd)
        msg = 'Cannot remove NIM resource \'{}\''.format(lpp_source)
        module.log('[WARNING] {}', msg)
        module.log('[WARNING] cmd:{} failed rc={} stdout:{} stderr:{}'
                   .format(cmd, rc, stdout, stderr))
...
def check_targets(module, output, targets, nim_clients):
    ...
    for machine in targets:
        ...
        if nim_clients[machine]['Cstate'] != 'ready for a NIM operation':
            module.log('[WARNING] {} is not ready for NIM operation'.format(machine))
            continue
        ...
        rc, stdout, stderr = module.run_command(cmd, use_unsafe_shell=True)
        if rc != 0:
            msg = 'Cannot check {} connectivity with c_rsh: command \'{}\' returned rc: {}'.format(machine, ' '.join(cmd), rc)
            module.log('[WARNING] ' + msg)
            output['messages'].append(msg)

        rc, stdout = compute_c_rsh_rc(machine, rc, stdout)
        if rc != 0:
            msg = 'Cannot reach {} with c_rsh, rc:{}, stderr:{}'.format(machine, rc, stderr)
            module.log('[WARNING] ' + msg)
...
def main():
    ...
    module.debug('*** OHAI ***')
    module.debug('requested targets are: "{}"'.format(targets))
    nim_clients = get_nim_clients_info(module)
    module.debug('Nim clients are: {}'.format(nim_clients))
    targets = expand_targets(module, targets, nim_clients.keys())
    module.debug('Nim client targets are:{}'.format(targets))
    ...
    module.debug('Available target machines are:{}'.format(targets))
    if not targets:
    ...
    if os.path.exists(flrtvc_path):
        try:
            ...
            msg = 'Cannot remove {}, exception:{}'.format(flrtvc_path, exc)
            module.log('[WARNING] ' + msg)
    ...
    for machine in wrong_targets:
        msg = 'Failed to get vulnerabilities report, {} will not be updated'.format(machine)
        module.log('[WARNING] ' + msg)

...
def create_ssh_client(host_info):
    ...
    arguments:
        host_info (dict): Connection parameters (host, username, port, key_path, password, timeout)
    note:
...
def get_file_checksum(ssh_client, file_path):
    """
    Calculate MD5 checksum of a file using the AIX-native csum command.
    Used to detect content changes in monitored files.

    arguments:
        ssh_client (paramiko.SSHClient): Active SSH connection to the AIX host
        file_path (str): Absolute path to the file on the remote AIX system
    note:
    ...
    stdin, stdout, stderr = ssh_client.exec_command(f"csum '{file_path}' 2>/dev/null")
...
def get_file_info(ssh_client, file_path):
    """
    ...
    try:
        ...
        stdin, stdout, stderr = ssh_client.exec_command(f"ls -la '{file_path}' 2>/dev/null")
...
def detect_changes(current_info, previous_info):
    """
    ...
    arguments:
        current_info (dict): Current file information from get_file_info()
        previous_info (dict or None): Previous file information, or None for new files
    note:
...
def create_file_event(hostname, file_path, action, current_info, previous_info, changes):
    """
    ...
    arguments:
        hostname (str): Hostname or IP of the AIX system where change occurred
        file_path (str): Absolute path to the changed file
        action (str): Type of action ("initial", "created", "modified")
        current_info (dict): Current file information from get_file_info()
        previous_info (dict or None): Previous file information, or None for new files
        changes (list): List of change types from detect_changes()
    note:
...
def create_deletion_event(hostname, file_path, previous_info):
    """
    Create a standardized event dictionary for file deletion events.
    Includes the previous state of the file before deletion.

    arguments:
        hostname (str): Hostname or IP of the AIX system where deletion occurred
        file_path (str): Absolute path to the deleted file
        previous_info (dict): Previous file information before deletion
    note:
...
async def scan_files_on_host(ssh_client, hostname, file_list, file_states, event_queue, emit_initial):
    """
    ...
    arguments:
        ssh_client (paramiko.SSHClient): Active SSH connection to the AIX host
        hostname (str): Hostname or IP of the AIX system being scanned
        file_list (list): List of absolute file paths to monitor
        file_states (dict): Dictionary to store file states (modified in place)
        event_queue (asyncio.Queue): Queue for emitting events to the rulebook
        emit_initial (bool): Whether to emit events for initial file states
    note:
...
async def check_file_changes(ssh_client, hostname, file_path, file_states, event_queue):
    """
    ...
    arguments:
        ssh_client (paramiko.SSHClient): Active SSH connection to the AIX host
        hostname (str): Hostname or IP of the AIX system
        file_path (str): Absolute path to the file being checked
        file_states (dict): Dictionary containing stored file states
        event_queue (asyncio.Queue): Queue for emitting events to the rulebook
    note:
...
async def monitor_host(host_info, file_list, file_states, event_queue):
    """
    Monitor all files on a single host during periodic checks.
    Creates SSH connection, checks all files, and handles errors.

    arguments:
        host_info (dict): Connection parameters for the host
        file_list (list): List of absolute file paths to monitor
        file_states (dict): Dictionary containing stored file states
        event_queue (asyncio.Queue): Queue for emitting events to the rulebook
    note:
...
async def main(queue: asyncio.Queue, args: Dict[str, Any]):
    """
    Main entry point for AIX File Watch event source plugin.
    Performs initial scan, then continuously monitors files at specified intervals.

    arguments:
        queue (asyncio.Queue): Event queue provided by ansible-rulebook
        args (Dict[str, Any]): Configuration parameters (hosts, files, interval, emit_initial)
    note:
    ...
    if not host_list:
        raise ValueError("AIXFileWatch: 'hosts' list is required.")

    file_list = args.get("files", [])
    if not file_list:
        raise ValueError("AIXFileWatch: 'files' list is required.")

    check_interval = int(args.get("interval", 30))
    emit_initial = bool(args.get("emit_initial", False))

    # Storage for file states
    # Structure: {hostname: {file_path: file_info}}
    file_states = {}
    ...
    for host_info in host_list:
        ...
        try:
            ...
            # Emit error event
            error_event = create_error_event(hostname, f"Initial connection failed: {str(e)}")

...
def list_versions(module):
    ...
    if "Another session of the tool is running" in stdout:
      results['msg'] = "Another session of the tool is running, please close it and try again."
      module.fail_json(**results)
...
def update_timezone_offline(module):
  ...
  if "Another session of the tool is running" in stdout:
    results['msg'] = "Another session of the tool is running, please close and try again."
    module.fail_json(**results)
  ...
  if "The system is already installed with" in stdout:
    ...
    payload["msg"] += " the system is already installed with the provided database version."
...
def main():
    ...
    module = AnsibleModule(
        ...
        argument_spec=dict(
            action=dict(type='str', choices=['list_versions', 'print_updated_zones', 'update_timezone', 'update_timezone_offline'], required=True),
    ...
    if not check_tzupg(module):
        results['msg'] += "Please check the AIX version. 'tzupg' tool is not available on the system."
        module.fail_json(**results)
    ...
    if action == "update_timezone":
        ...
        if not tz:
            results['msg'] = "You need to provide the timezone parameter for action 'update_timezone'"
            module.fail_json(**results)
      ...
      if not is_new:
        results['msg'] = "This functionality is only available for version '2537A_73H' and higher."
        results['msg'] += " Please check the version and try again."
        module.fail_json(**results)
      
      if not module.params['db_location']:
        results['msg'] = "You need to provide 'db_location' for updating timezone offline."
        module.fail_json(**results)
      
      if not check_db_exists(module):
        results['msg'] = "The database does not exist at the provided location. Please check and retry."
        module.fail_json(**results)

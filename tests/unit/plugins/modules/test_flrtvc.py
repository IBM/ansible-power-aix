...
# it is used to check the "parse" part of "meta" in the report for a protocol.
...
class TestRunParser(unittest.TestCase):
    ...
    def test_http_to_ftp(self):
        ...
        flrtvc.results['meta'].update({'report': mock_http})
        flrtvc.run_parser(flrtvc.results['meta']['report'], self.localpatchserver, self.localpatchpath)
        self.assertTrue(_check_protocol('ftp', flrtvc.results['meta']['parse']))
    ...
    def test_http_to_https(self):
        ...
        flrtvc.results['meta'].update({'report': mock_http})
        flrtvc.run_parser(flrtvc.results['meta']['report'], self.localpatchserver, self.localpatchpath)
        self.assertTrue(_check_protocol('https', flrtvc.results['meta']['parse']))

    def test_ftp_to_http(self):
        self.module.params['protocol'] = "http"
        flrtvc.module = self.module
        flrtvc.results = self.results
        flrtvc.results['meta'].update({'report': mock_ftp})
        flrtvc.run_parser(flrtvc.results['meta']['report'], self.localpatchserver, self.localpatchpath)
        self.assertTrue(_check_protocol('http', flrtvc.results['meta']['parse']))
    ...
    def test_https_to_ftp(self):
        ...
        flrtvc.module = self.module
        flrtvc.results = self.results
        flrtvc.results['meta'].update({'report': mock_https})
        flrtvc.run_parser(flrtvc.results['meta']['report'], self.localpatchserver, self.localpatchpath)
        self.assertTrue(_check_protocol('ftp', flrtvc.results['meta']['parse']))
